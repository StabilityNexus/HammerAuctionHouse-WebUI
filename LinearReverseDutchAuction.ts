...
export interface LinearReverseDutchAuction {
  name: string;
  desc: string;
  imgUrl: string;
  auctionType: bigint;
...
export async function getAllAuctions(client: any) {
  try {
    ...
    const logs = await client.getLogs({
      address: contractAddress,
    event: parseAbiItem(
      'event AuctionCreated(uint256 indexed Id,string name,string description,string imgUrl,address auctioneer,uint8 auctionType,address auctionedToken,uint256 auctionedTokenIdOrAmount,address biddingToken,uint256 startingPrice,uint256 reservedPrice,uint256 deadline)'
    ),
      fromBlock,
