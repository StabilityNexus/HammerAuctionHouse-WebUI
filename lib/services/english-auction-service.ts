...
// ABI for English Auction contract - includes auctionCounter and auctions functions
const ENGLISH_ABI = [
  {
    "inputs": [],
    "name": "auctionCounter",
...
export class EnglishAuctionService implements IAuctionService {
  ...
  async createAuction(writeContract: any, params: EnglishAuctionParams): Promise<void> {
    throw new Error("English auction service not yet implemented");
  }
  ...
  async getAllAuctions(client: any, startBlock: bigint, endBlock: bigint): Promise<any[]> {
    try {
      ...
      throw new Error("English auction service event-based fetching not yet implemented");
    } catch (error) {
